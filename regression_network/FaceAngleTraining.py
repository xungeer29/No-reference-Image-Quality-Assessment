...
class Face_Angle():
    def __init__(self, imgsize=128, batchsize=64, weight_decay = 0.0002, train = True, load_model_dir = 'load' ,save_model_dir = 'save' ):
        ...
        self.regression_label = tf.placeholder(tf.float32, [self.batch_size, 2]) # **************************
    ...
    def train_listfile(self, regression_iter, epoch_index):
        ...
        for i in range(int(len(self.regression_list) / self.batch_size)-1):
            ...
            label = label.reshape([self.batch_size, 2]) # 回归的label个数 ############*********************
            regression_loss, _ = self.sess.run([self.regression_loss, self.regression_optim],
    ...
    def train(self):
        ...
        def read_regression_image(sample):
            ...
            # confidence_3 = sample[4]

            con = [confidence_0, confidence_1]
    ...
    def read_quality_list(self, txt_path):
        ...
        for line in lines:
            ...
            samples.append([image_path, yaw, pitch])
