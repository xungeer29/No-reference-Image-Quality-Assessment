...
class Face_Angle():
    def __init__(self, imgsize=128, batchsize=64, weight_decay = 0.0002, train = True, load_model_dir = 'load' ,save_model_dir = 'save' ):
        ...
        """
        # balance the pose loss and the qua loss
        pose_logits = self.regression_logits[:, 0:2]
        qua_logits = self.regression_logits[:, 2]
        qua_logits = tf.reshape(tf.multiply(qua_logits, loss_weight), [self.batch_size, 1])
        balance_logits = tf.concat([pose_logits, qua_logits], 1)
        """

        # self.regression_loss = MSE_loss(self.regression_logits , self.regression_label) # *****************
        self.regression_loss_pose = MSE_loss(self.regression_logits[:, 0:2], self.regression_label[:, 0:2]) # *****************
        self.regression_loss_qua = MSE_loss(self.regression_logits[:, 2], self.regression_label[:, 2]) # *****************
        self.regression_loss = self.regression_loss_pose + loss_weight*self.regression_loss_qua # balance the pose loss and the qua loss
    ...
    def train_listfile(self, regression_iter, epoch_index):
        ...
        for i in range(int(len(self.regression_list) / self.batch_size)-1):
            ...
            label = label.reshape([self.batch_size, 3]) # 回归的label个数 ############*********************
            pose_loss, qua_loss, regression_loss, _ = self.sess.run([self.regression_loss_pose, 
                                            self.regression_loss_qua,self.regression_loss, self.regression_optim],
            ...
            print pose_loss, qua_loss, regression_loss 
            f.write('pose_loss:{} qua_loss:{} regression_loss:{}\n'.format(pose_loss, qua_loss, regression_loss))
    ...
    def train(self):
        ...
        def read_regression_image(sample):
            path = sample[0]
            confidence_0 = sample[1]
            confidence_1 = sample[2]
            confidence_2 = sample[3]

            con = [confidence_0, confidence_1, confidence_2]
    ...
    def read_quality_list(self, txt_path):
        ...
        for line in lines:
            ...
            qua = float(qua) # 放缩到0-10
            qua = qua**w1*w2

            samples.append([image_path, yaw, pitch, qua])
...
if __name__ == '__main__':
    ...
    #f = open('/data2/gaofuxun/data/RankIQA/'+'weight='+str(loss_weight)+'.txt', 'w')
    ...
    testpath = '/data2/gaofuxun/data/RankIQA/iqiyi_tid2013_128_test_'+str(loss_weight)
    ...
    for root, dirs, files in os.walk(test_image_path):
        for name in files:
            if name.endswith('.jpg'):
                ...
                qua_score = (logits[2]/w2)**(1/w1)
                #cv2.imwrite(testpath+'/'+str(logits[2])+'.jpg', img_row)
                cv2.imwrite(testpath+'/'+str(qua_score)+'.jpg', img_row)
